...
def _getInsightlyProject(project):
    return get(INSIGHTLY_PROJECTS_URI + str(project['o']), auth=(INSIGHTLY_API_KEY, '')).json()


def _getNextStage(insightly_project):
    return filter(lambda s:
                  s['STAGE_ORDER'] in map(lambda o: o['STAGE_ORDER'] + 1,
                                          filter(lambda f: f['STAGE_ID'] == insightly_project['STAGE_ID'],
                                                 filter(lambda s: s['PIPELINE_ID'] == insightly_project['PIPELINE_ID'],
                                                        STAGES))) and
                  s['PIPELINE_ID'] == insightly_project['PIPELINE_ID'], STAGES)[0]['STAGE_ID']


def createDefaultTenantFor(project):
    ...
    tenant = post(INSIGHTLY_PROJECTS_URI,
                  data=json.dumps(payload),
                  headers={'Content-Type': 'application/json'},
                  auth=(INSIGHTLY_API_KEY, '')).json()

    parent['LINKS'] = parent['LINKS'] + [{'SECOND_PROJECT_ID': tenant['PROJECT_ID']}]

    put(INSIGHTLY_PROJECTS_URI,
        data=json.dumps(parent),
        headers={'Content-Type': 'application/json'},
        auth=(INSIGHTLY_API_KEY, ''))

    return tenant


def updateProject(project, updateStage=True, status=None):
